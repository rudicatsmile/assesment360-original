...
<body class="min-h-screen bg-zinc-100 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
    <div class="flex min-h-screen" x-data="{ dark: document.documentElement.classList.contains('dark') }">
        <aside class="w-72 border-r border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <nav class="space-y-2" aria-label="Admin Navigation">
                <a href="{{ route('admin.dashboard') }}" wire:navigate class="block">
                    <flux:button :variant="request()->routeIs('admin.dashboard') ? 'primary' : 'ghost'" icon="home"
                ...
                @if(auth()->user()?->hasPermission('view_analytics'))
                    <a href="{{ route('admin.analytics.index') }}" wire:navigate class="block">
                        <flux:button :variant="request()->routeIs('admin.analytics.*') ? 'primary' : 'ghost'"
                            icon="chart-bar" class="w-full justify-start">
                            Analytics
                        </flux:button>
                    </a>
                @endif
                ...
                @if(auth()->user()?->hasPermission('manage_departments'))
                    <a href="{{ route('admin.departments.index') }}" wire:navigate class="block">
                        <flux:button :variant="request()->routeIs('admin.departments.*') ? 'primary' : 'ghost'"
                            icon="building-office" class="w-full justify-start">
                            Departments
                        </flux:button>
                    </a>
                @endif
                ...
                @if(auth()->user()?->hasPermission('manage_users'))
                    <a href="{{ route('admin.users.index') }}" wire:navigate class="block">
                        <flux:button :variant="request()->routeIs('admin.users.*') ? 'primary' : 'ghost'" icon="users"
                            class="w-full justify-start">
                            Users
                        </flux:button>
                    </a>
                ...
                @if(auth()->user()?->hasPermission('manage_roles'))
                    <a href="{{ route('admin.roles.index') }}" wire:navigate class="block">
                        <flux:button :variant="request()->routeIs('admin.roles.*') ? 'primary' : 'ghost'"
                            icon="shield-check" class="w-full justify-start">
                            Role Management
                        </flux:button>
                    </a>
                @endif
            ...
            <div class="mt-6 space-y-2">
                <flux:button variant="outline" icon="moon" class="w-full justify-start" x-on:click="
                        dark = !dark;
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <flux:button type="submit" variant="danger" icon="arrow-right-start-on-rectangle"
                        class="w-full justify-start">
                        Logout
                    </flux:button>
                </form>
